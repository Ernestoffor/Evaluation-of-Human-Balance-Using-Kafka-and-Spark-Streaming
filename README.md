# STEDI Ecosystem
This project is all about working in a data science team at STEDI, a small startup focused on assessing balance for seniors. STEDI has an application that collects data from seniors during a small exercise. The user logs in, and then selects the customer they are working with. Then the user starts a timer, and clicks a button with each step the senior takes. When the senior has reached 30 steps, their test is finished. The data transmitted enables the application to monitor seniors’ balance risk. 
...
- There are 3 Spark Python scripts. Each will connect to a kafka broker running at `kafka:19092` :
    - `redis-server` topic: Thee spark script `sparkpyrediskafkastreamtoconsole.py` subscribes to the `redis-server` topic, base64 decoded the payload, and deserialized the JSON to individual fields, then print the fields to the console. The data includes the birth date and email address.
    - `stedi-events` topic: The second spark script `sparkpyeventskafkastreamtoconsole.py`  subscribes to the `stedi-events` topic and deserialized the JSON (it is not base64 encoded) to individual fields. This produces email address and the risk score.
    - `risk-output-stedi-event-score` ( a new Topic): The spark script `sparkpykafkajoin.py` to join the customer dataframe and the customer risk dataframes, joining on the email address. A JSON output is created to the newly created kafka topic configured for STEDI to subscribe to that contains  the following fields below:
...
```
The data sinked to this new topic is responsible for producing dynamic data points in the graph.
...
- Uploaded screenshots of the working graph are inside the screenshots workspace folder 
